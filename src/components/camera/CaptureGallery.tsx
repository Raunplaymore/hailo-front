    ...
    <Card>
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <h2 className="text-lg font-semibold text-foreground">최근 촬영물</h2>
        </div>
      </div>
      {items.length === 0 ? (
        <p className="text-sm text-muted-foreground bg-muted/35 border border-border rounded-lg px-3 py-2">
          아직 촬영물이 없습니다.
        </p>
      ) : (
        <div className="grid gap-3">
          {items.map((item) => (
            <div
              key={item.filename}
              className="rounded-xl border border-border bg-muted/30 shadow-sm p-3 space-y-2"
            >
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-foreground break-words">{item.filename}</p>
